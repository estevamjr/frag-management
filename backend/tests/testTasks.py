...
def test_split_log_into_match_chunks():
    ...
    assert os.path.exists(
        SAMPLE_LOG_FILE
    ), "O arquivo sample_log.txt não foi encontrado na pasta 'tests/'"
    ...
    assert (
        len(matches) == 2
    ), f"Esperado 2 partidas, mas {len(matches)} foram encontradas"
    ...
    assert (
        match_2["end_time"] is not None
    ), "O end_time da segunda partida (extraído da última linha) não foi pego"
