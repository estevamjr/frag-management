...
for i in range(NUM_MATCHES):
    ...
    log_lines.append(
        f"{start_time_match.strftime('%d/%m/%Y %H:%M:%S')} - New match {match_id} has started"
    )

    # Gera Players para esta partida
    players_list = [
        f"Player_{uuid.uuid4().hex[:4]}"
        for _ in range(random.randint(*PLAYERS_PER_MATCH))
    ]
    ...
    for k in range(num_kills):
        ...
        victim = random.choice(
            [p for p in players_list if p != killer]
        )  # Garante que não se mate
        weapon = random.choice(WEAPONS)
        log_lines.append(
            f"{kill_time.strftime('%d/%m/%Y %H:%M:%S')} - {killer} killed {victim} using {weapon}"
        )

    # Linha de Fim da Partida (10 minutos depois do início)
    end_time_match = start_time_match + timedelta(minutes=10)
    log_lines.append(
        f"{end_time_match.strftime('%d/%m/%Y %H:%M:%S')} - Match {match_id} has ended"
    )
...
print(
    f"Sucesso! Arquivo '{file_name}' criado com {NUM_MATCHES} partidas simuladas no formato de log."
)
