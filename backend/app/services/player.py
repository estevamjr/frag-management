...
def get_player_by_id(db: Session, player_id: uuid.UUID):
    ...
    """
    return (
        db.query(match_models.Player)
        .filter(match_models.Player.id == player_id)
        .first()
    )
...
def get_player_by_db_id(db: Session, match_db_id: uuid.UUID):
    ...
    return (
        db.query(match_models.Player)
        .filter(match_models.Player.id == match_db_id)
        .first()
    )
...
def get_all_players(
    ...
    query = db.query(match_models.Player).options(
        joinedload(match_models.Player.bonuses)
    )
